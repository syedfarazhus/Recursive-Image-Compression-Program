...
class Decompressor:
    ...
    def decompress(qd_tree: QuadTree) -> List[List[int]]:
        ...
        """
        pixels = qd_tree.convert_to_pixels()
...
if __name__ == '__main__':
    ...
    while True:
        abort = input("Abort: Y or N: ")
        if abort.lower() == "y":
            break

        runtype = input("C --> Compress OR D --> Decompress: ").lower()
        if not (runtype == "c" or runtype == "d"):
            print("Invalid, input")
            continue
